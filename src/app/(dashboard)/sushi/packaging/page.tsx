...
const DEFAULT_FORM = { name: "", quantity: 0, unit: "个", notes: "" };
...
export default function PackagingPage() {
  ...
  const [modal, setModal] = useState<{ mode: ModalMode; item?: PackagingItem } | null>(null);
  ...
  async function load() {
    const res = await fetch("/api/sushi/packaging");
  ...
  async function handleSave() {
    ...
    try {
      const url = modal?.mode === "add" ? "/api/sushi/packaging" : `/api/sushi/packaging/${modal?.item?.id}`;
  ...
  async function handleDelete(id: string) {
    await fetch(`/api/sushi/packaging/${id}`, { method: "DELETE" });
    setDeleteId(null); load();
  }

  async function adjustQty(item: PackagingItem, delta: number) {
    const newQty = Math.max(0, item.quantity + delta);
    setAdjusting(prev => ({ ...prev, [item.id]: true }));
    await fetch(`/api/sushi/packaging/${item.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ quantity: newQty }) });
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">共 {items.length} 种包装</p>
        </div>
        <button onClick={openAdd} className="flex items-center gap-2 bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
          <Plus size={16} /> 添加品项
        </button>
      </div>
                ...
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <span className={`min-w-[3.5rem] text-center font-bold text-lg ${item.quantity === 0 ? "text-red-500" : item.quantity <= 20 ? "text-amber-500" : "text-slate-800"}`}>
                      ...
                      <button onClick={() => adjustQty(item, 1)} disabled={adjusting[item.id]}
                        className="w-7 h-7 rounded-full bg-purple-100 hover:bg-purple-200 flex items-center justify-center text-purple-600 disabled:opacity-40 transition-colors">
                        <Plus size={12} />
                      </button>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => openEdit(item)} className="p-1.5 text-slate-400 hover:text-purple-500 transition-colors"><Pencil size={14} /></button>
      ...
      {items.some(i => i.quantity > 0 && i.quantity <= 20) && (
        <div className="mt-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700">
          ⚡ 库存偏低（≤20）：{items.filter(i => i.quantity > 0 && i.quantity <= 20).map(i => `${i.name}(${i.quantity}${i.unit})`).join("、")}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
            <div className="px-6 py-5 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-800">{modal.mode === "add" ? "添加包装" : `编辑 — ${modal.item?.name}`}</h2>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">名称 *</label>
                <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  placeholder="例：寿司盒 8格" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-purple-400" />
              </div>
              <div className="flex gap-3">
                ...
                <div className="w-24">
                  ...
                  <input type="text" value={form.unit} onChange={e => setForm(f => ({ ...f, unit: e.target.value }))}
                    placeholder="个/箱/包" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-purple-400" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">备注（可选）</label>
                <input type="text" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                  placeholder="例：仓库第三排" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-purple-400" />
              </div>
            </div>
            ...
            <div className="px-6 py-4 border-t border-slate-100 flex gap-3 justify-end">
              ...
              <button onClick={handleSave} disabled={saving || !form.name.trim()}
                className="px-5 py-2 bg-purple-500 hover:bg-purple-600 text-white text-sm font-medium rounded-lg disabled:opacity-50 transition-colors">
                {saving ? "保存中..." : "保存"}
              </button>
            </div>
          </div>
