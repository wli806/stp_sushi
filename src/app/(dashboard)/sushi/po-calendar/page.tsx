...
const DOW_EN_SHORT = ["M","T","W","T","F","S","S"];
...
export default function POCalendarPage() {
  ...
  const DOW_NAMES_SHORT = lang === "en" ? DOW_EN_SHORT : DOW_ZH;
    ...
    <div className="p-4 md:p-8">
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-l border-slate-100">
          {cells.map((day, i) => {
            if (!day) return <div key={i} className="min-h-[56px] sm:min-h-[80px] md:min-h-[110px] border-r border-b border-slate-100 bg-slate-50/60" />;
            ...
            return (
              <div key={i} className={`min-h-[56px] sm:min-h-[80px] md:min-h-[110px] border-r border-b border-slate-100 p-0.5 sm:p-1.5 ${isPast && dayOrders.some(o => o.status === 1) ? "bg-red-50/40" : ""}`}>
                <div className={`text-[10px] sm:text-xs font-medium mb-0.5 sm:mb-1 w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center rounded-full mx-auto ${isToday ? "bg-orange-500 text-white" : "text-slate-500"}`}>
                  {day}
                </div>
                    ...
                    if (isPending) {
                      return (
                        <a key={order.id} href={url} target="_blank" rel="noopener noreferrer"
                          ...
                          className={`block text-[9px] sm:text-xs rounded px-1 sm:px-1.5 py-0.5 truncate leading-4 sm:leading-5 cursor-pointer transition-all hover:ring-2 hover:ring-offset-1 hover:shadow-md font-medium ${isOverdue ? "bg-red-500 text-white hover:ring-red-500" : "bg-orange-400 text-white hover:ring-orange-400"}`}>
                          <span className="inline-flex items-center gap-0.5 w-full truncate">
                            <ExternalLink size={7} className="flex-shrink-0 hidden sm:inline" />
                      ...
                      return (
                        <div key={order.id}
                          className="text-[9px] sm:text-xs rounded px-1 sm:px-1.5 py-0.5 truncate leading-4 sm:leading-5 bg-green-100 text-green-700 font-medium"
                          title={`${t("pocal.ordered")}: ${order.supplierName}`}>
                          <span className="inline-flex items-center gap-0.5 w-full truncate">
                            <CheckCircle2 size={7} className="flex-shrink-0 hidden sm:inline" />
                            <span className="truncate">{supplierShort(order.supplierName)}</span>
                          </span>
                        </div>
                      );
