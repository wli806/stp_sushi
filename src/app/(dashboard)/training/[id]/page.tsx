...
import { ChevronLeft, Plus, Pencil, Trash2, CheckCircle2, Circle, GraduationCap } from "lucide-react";
...
const CHECKLIST_AREAS = [
  ...
  {
    key: "sushi",
    labelZh: "寿司制作", labelEn: "Sushi Production",
    tasks: [
  ...
  {
    key: "cs",
    labelZh: "客户服务", labelEn: "Customer Service",
    tasks: [
  ...
  {
    key: "ops",
    labelZh: "店铺运营", labelEn: "Store Operations",
    tasks: [
...
// ── Types ─────────────────────────────────────────────────────────────────────
interface ChecklistItem { taskKey: string; completedAt: string; trainerName: string; }
...
const EMPTY_RECORD = { date: "", type: "Feedback", areaForImprovement: "", feedbackActionTaken: "", followUpNeeded: "" };
...
export default function TrainingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Checklist sign-off modal
  const [signTask, setSignTask] = useState<{ key: string; label: string } | null>(null);
  const [signForm, setSignForm] = useState({ completedAt: new Date().toISOString().slice(0, 10), trainerName: "" });
  ...
  const checklistMap = new Map((staff?.checklist ?? []).map(c => [c.taskKey, c]));
  const completedCount = staff?.checklist.length ?? 0;

  // ── Record actions ──────────────────────────────────────────────────────────
  function openAddRecord() { setEditingRecord(null); setRecordForm({ ...EMPTY_RECORD }); setShowRecordModal(true); }
  ...
  async function handleSignOff() {
    if (!signTask || !signForm.completedAt || !signForm.trainerName) return;
    setSigningKey(signTask.key);
    await fetch(`/api/training/staff/${id}/checklist`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ taskKey: signTask.key, completedAt: signForm.completedAt, trainerName: signForm.trainerName }),
    });
    setSigningKey(null);
    setSignTask(null);
    load();
  ...
  async function handleUnsign(taskKey: string) {
    if (!confirm(t("training.checklist.confirmUnsign"))) return;
    setSigningKey(taskKey);
    await fetch(`/api/training/staff/${id}/checklist`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ taskKey, completedAt: "", trainerName: "" }),
    });
    setSigningKey(null);
    load();
  }
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="flex gap-1 bg-slate-100 rounded-lg p-1 mb-6 w-fit">
        {(["records", "checklist"] as const).map(tb => (
          <button key={tb} onClick={() => setTab(tb)}
            ...
            {tb === "checklist" && <span className="ml-1.5 text-xs text-orange-500">{completedCount}/{TOTAL_TASKS}</span>}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          {staff.records.length === 0 ? (
            ...
            <div className="space-y-3">
              {staff.records.map(r => (
                <div key={r.id} className={`bg-white rounded-xl border border-slate-100 shadow-sm border-l-4 ${roleBorder(r.createdByRole)} p-4 md:p-5`}>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        {r.createdByUsername && (
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${roleBadge(r.createdByRole)}`}>
                            {r.createdByUsername}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      <button onClick={() => openEditRecord(r)} className="p-1.5 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-colors"><Pencil size={13} /></button>
                      <button onClick={() => handleDeleteRecord(r.id)} className="p-1.5 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={13} /></button>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div>
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 mb-5">
            <div className="flex items-center justify-between text-sm mb-2">
              ...
              <span className={`font-bold ${completedCount === TOTAL_TASKS ? "text-green-600" : "text-orange-500"}`}>{completedCount}/{TOTAL_TASKS}</span>
            </div>
            <div className="w-full bg-slate-100 rounded-full h-2">
              <div className={`h-2 rounded-full transition-all ${completedCount === TOTAL_TASKS ? "bg-green-500" : "bg-orange-400"}`}
                style={{ width: `${Math.round((completedCount / TOTAL_TASKS) * 100)}%` }} />
            </div>
          </div>
          ...
          {CHECKLIST_AREAS.map(area => (
            <div key={area.key} className="bg-white rounded-xl border border-slate-100 shadow-sm mb-4 overflow-hidden">
              ...
              <div className="divide-y divide-slate-50">
                {area.tasks.map(task => {
                  const done = checklistMap.get(task.key);
                  return (
                    <div key={task.key} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm ${done ? "text-slate-600" : "text-slate-700"}`}>{task.labelEn}</p>
                        {done && (
                          <p className="text-xs text-slate-400 mt-0.5">{done.completedAt} · {done.trainerName}</p>
                        )}
                      </div>
                      <div className="flex-shrink-0">
                        {done ? (
                          <button onClick={() => handleUnsign(task.key)} disabled={signingKey === task.key}
                            className="text-xs text-slate-400 hover:text-red-500 transition-colors px-2 py-1 rounded hover:bg-red-50">
                            {t("training.checklist.unsign")}
                          </button>
                        ) : (
                          <button
                            onClick={() => { setSignTask({ key: task.key, label: task.labelEn }); setSignForm({ completedAt: new Date().toISOString().slice(0, 10), trainerName: staff.trainerName || "" }); }}
                            disabled={signingKey === task.key}
                            className="text-xs bg-orange-500 hover:bg-orange-600 text-white px-2.5 py-1 rounded-lg transition-colors font-medium">
                            {t("training.checklist.signOff")}
                          </button>
                        )}
                      </div>
                    </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="date" className={inputCls} value={recordForm.date} onChange={e => setRecordForm(f => ({ ...f, date: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs text-slate-500 mb-1">{t("training.record.type")}</label>
                <select className={inputCls} value={recordForm.type} onChange={e => setRecordForm(f => ({ ...f, type: e.target.value }))}>
            ...
            <div>
              ...
              <textarea className={`${inputCls} resize-none`} rows={2} value={recordForm.areaForImprovement} onChange={e => setRecordForm(f => ({ ...f, areaForImprovement: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">{t("training.record.feedback")} *</label>
              <textarea className={`${inputCls} resize-none`} rows={2} value={recordForm.feedbackActionTaken} onChange={e => setRecordForm(f => ({ ...f, feedbackActionTaken: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">{t("training.record.followUp")} *</label>
              <textarea className={`${inputCls} resize-none`} rows={2} value={recordForm.followUpNeeded} onChange={e => setRecordForm(f => ({ ...f, followUpNeeded: e.target.value }))} />
            </div>
            <div className="flex gap-3 pt-1">
              <button onClick={() => setShowRecordModal(false)} className="flex-1 border border-slate-200 text-slate-600 py-2 rounded-lg text-sm hover:bg-slate-50 transition-colors">{t("common.cancel")}</button>
              <button onClick={handleSaveRecord} disabled={savingRecord || !recordForm.date || !recordForm.areaForImprovement || !recordForm.feedbackActionTaken || !recordForm.followUpNeeded}
      ...
      {/* ── Sign-off Modal ───────────────────────────────────────────────────── */}
      {signTask && (
        <Modal title={t("training.checklist.signOff")} onClose={() => setSignTask(null)}>
          <div className="space-y-4">
            <p className="text-sm text-slate-600 bg-slate-50 rounded-lg px-3 py-2">{signTask.label}</p>
            <div>
              <label className="block text-xs text-slate-500 mb-1">{t("training.record.date")} *</label>
              <input type="date" className={inputCls} value={signForm.completedAt} onChange={e => setSignForm(f => ({ ...f, completedAt: e.target.value }))} />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">{t("training.form.trainer")} *</label>
              <input className={inputCls} value={signForm.trainerName} onChange={e => setSignForm(f => ({ ...f, trainerName: e.target.value }))} />
            </div>
            <div className="flex gap-3 pt-1">
              <button onClick={() => setSignTask(null)} className="flex-1 border border-slate-200 text-slate-600 py-2 rounded-lg text-sm hover:bg-slate-50 transition-colors">{t("common.cancel")}</button>
              <button onClick={handleSignOff} disabled={!signForm.completedAt || !signForm.trainerName}
                className="flex-1 bg-green-600 hover:bg-green-700 disabled:bg-green-300 text-white py-2 rounded-lg text-sm font-medium transition-colors">
                {t("training.checklist.signOff")}
              </button>
            </div>
          </div>
